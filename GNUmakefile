...
clean:
	$(MAKE) -C kernel clean
	rm -rf iso_root boot.log storage-boot.log fault-boot.log divide-fault-boot.log page-fault-boot.log gp-fault-boot.log double-fault-boot.log $(STORAGE_TEST_DISK) $(STORAGE_TEST_IMAGE).iso $(IMAGE).iso $(FAULT_IMAGE).iso $(DIVIDE_FAULT_IMAGE).iso $(PAGE_FAULT_IMAGE).iso $(GP_FAULT_IMAGE).iso $(DOUBLE_FAULT_IMAGE).iso
