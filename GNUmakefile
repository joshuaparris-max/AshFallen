...
.PHONY: all kernel host-tests run smoke fault-smoke ring3-smoke clean distclean
all: $(IMAGE).iso
...
clean:
	$(MAKE) -C kernel clean
	rm -rf iso_root boot.log fault-boot.log divide-fault-boot.log page-fault-boot.log gp-fault-boot.log double-fault-boot.log ring3-boot.log $(IMAGE).iso $(FAULT_IMAGE).iso $(DIVIDE_FAULT_IMAGE).iso $(PAGE_FAULT_IMAGE).iso $(GP_FAULT_IMAGE).iso $(DOUBLE_FAULT_IMAGE).iso
